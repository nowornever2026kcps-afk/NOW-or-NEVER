export function initUpdates({ supabaseClient, $, sectionContent, adminToast, escapeHTML, loadOverview }) {
  ...
  function formHTML(item = null) {
    const isEdit = !!item;
    return `
      <div class="updates-editor">
        <div class="updates-editor-head">
          <div><p class="eyebrow">${isEdit ? 'EDIT UPDATE' : 'NEW UPDATE'}</p><h4>${isEdit ? 'Edit announcement' : 'Publish an update'}</h4></div>
          ${isEdit ? '<button class="ghost-btn" type="button" id="cancelUpdateEdit">Cancel</button>' : ''}
        </div>
        ...
        <div class="updates-form-actions">
          <button class="primary-btn" type="button" id="saveUpdateBtn">${isEdit ? 'Save changes' : 'Publish update'}</button>
          <span class="muted">${isEdit ? 'Changes are protected by server-side admin checks.' : 'You can keep it unpublished as a draft.'}</span>
        </div>
      </div>`;
  }
  ...
  function listHTML() {
    ...
    return `<div class="updates-list">${updates.map(u => `
      <article class="admin-update-item ${u.enabled ? '' : 'is-disabled'}">
        <div class="admin-update-icon">${escapeHTML(u.icon || '📢')}</div>
        <div class="admin-update-main">
          ...
          <p>${escapeHTML(u.content).replaceAll('\n', '<br>')}</p>
          <small>${u.published_at ? new Date(u.published_at).toLocaleString('en-IN') : 'Not published'}</small>
        </div>
  ...
  async function renderUpdates() {
    try {
      await loadUpdates();
      sectionContent.innerHTML = `<div class="section-heading"><p class="eyebrow">ADMIN MODULE</p><h3>📢 Updates</h3><p class="muted">Create and control announcements shown to students.</p></div>${formHTML()}${listHTML()}`;
      bind();
    } catch (error) {
      console.error('Updates load failed:', error);
      sectionContent.innerHTML = '<div class="section-heading"><h3>📢 Updates</h3><p class="muted">Unable to load updates.</p></div>';
      adminToast(error.message || 'Unable to load updates');
    }
  }
  ...
  function bind() {
    ...
    sectionContent.querySelectorAll('[data-edit-update]').forEach(btn => btn.addEventListener('click', () => {
      editingId = Number(btn.dataset.editUpdate);
      const item = updates.find(x => Number(x.id) === editingId);
      sectionContent.innerHTML = `<div class="section-heading"><p class="eyebrow">ADMIN MODULE</p><h3>📢 Updates</h3></div>${formHTML(item)}${listHTML()}`;
      bind();
      $('updTitle')?.focus();
    }));
  ...
  async function saveUpdate() {
    ...
    const args = {
      p_title: title,
      p_content: content,
      p_update_type: $('updType')?.value || 'announcement',
      p_icon: $('updIcon')?.value.trim() || '📢',
      p_pinned: $('updPinned')?.checked || false,
      p_enabled: $('updEnabled')?.checked ?? true,
      p_publish: $('updPublish')?.checked ?? true
    };
    try {
      const result = editingId
        ? await supabaseClient.rpc('admin_updates_update', { p_id: editingId, ...args })
        : await supabaseClient.rpc('admin_updates_create', args);
      if (result.error) throw result.error;
      editingId = null;
      adminToast(editingId ? 'Update saved.' : 'Update created.', true);
      await renderUpdates();
      loadOverview?.();
    } catch (error) {
      console.error('Update save failed:', error);
      adminToast(error.message || 'Unable to save update');
    }
  }
  ...
  async function deleteUpdate(id) {
    ...
    try {
      const { error } = await supabaseClient.rpc('admin_updates_delete', { p_id: id });
      if (error) throw error;
      adminToast('Update deleted.', true);
      await renderUpdates();
      loadOverview?.();
    } catch (error) {
      console.error('Update delete failed:', error);
      adminToast(error.message || 'Unable to delete update');
    }
  }
