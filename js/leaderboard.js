  ...
  try{
    ...
    try{
      ...
      if(cosmeticError){
        ...
        (rows||[]).forEach(row=>{
          if(!cosmeticsByUser.has(row.user_id)) cosmeticsByUser.set(row.user_id,{});
    ...
    function renderLeaderboardCosmetics(student){
      ...
      if(accessory&&!isCrown(accessory)){
        visualAccessories.push(`<span class="leader-cosmetic leader-accessory" title="${escapeHtml(accessory.name)}">${escapeHtml(accessory.preview)}</span>`);
      }
    ...
    if(window.NowOrNeverDragon){
      document.querySelectorAll("[data-dragon-host='true']").forEach(host=>window.NowOrNeverDragon.mount(host));
    }
  }catch(err){
...
async function renderGroupStats(students,totalPoints,totalHours){
  ...
  $("groupStats").innerHTML=`
    ...
    <div class="stat"><div class="label">Points</div><div class="value">${totalPoints.toFixed(0)}</div></div>
    <div class="stat"><div class="label">Study hours</div><div class="value">${totalHours.toFixed(1)}</div></div>
    <div class="stat"><div class="label">Tasks done</div><div class="value">${totalTasks}</div></div>
  `;
}
