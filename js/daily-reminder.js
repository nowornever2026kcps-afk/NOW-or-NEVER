...
(() => {
  ...
  function notify() {
    ...
    if (typeof window.showToast === 'function') {
      window.showToast('🎯 Time to update your points!');
    }
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      #nonReminderCard .nr-title {
        font-weight: 800;
        font-size: 16px;
      }
      #nonReminderCard .nr-note {
        font-size: 12px;
        opacity: .72;
        margin-top: 4px;
      }
      #nonReminderCard .nr-controls {
        display: flex;
        align-items: center;
        gap: 8px;
        flex-wrap: wrap;
      ...
      #nonReminderCard input[type=time] {
        padding: 9px 11px;
        border-radius: 10px;
  ...
  function renderMoreMenuButton() {
    ...
    button.innerHTML = `
      ...
      <span>
        ...
        <small>Set your points reminder</small>
      </span>
    `;
    ...
    button.addEventListener('click', event => {
      ...
      if (meButton) {
        ...
        if (personal) {
          document.querySelectorAll('#mainApp .view').forEach(view => {
        ...
        if (card) {
          ...
          card.animate(
            [
              ...
              { transform: 'scale(1.015)', boxShadow: '0 0 28px rgba(124,92,255,.28)' },
              { transform: 'scale(1)', boxShadow: '0 0 0 rgba(124,92,255,0)' }
            ],
  ...
  function render() {
    ...
    card.innerHTML = `
      <div class="nr-row">
        <div>
          <div class="nr-title">⏰ Daily Points Reminder</div>
          <div class="nr-note">Remind me to add today's points and study progress.</div>
        </div>
        <label>
          <input id="nrEnabled" type="checkbox">
          Enable
        </label>
      </div>
      ...
      <div class="nr-controls">
        ...
        >
        <button id="nrSave" class="nr-primary" type="button">Save reminder</button>
        <button id="nrSkip" class="nr-secondary" type="button">Skip / Disable</button>
      </div>

      <div id="nrStatus" class="nr-status">
        Optional — you can leave this disabled.
      </div>
    `;
    ...
    card.querySelector('#nrSave').addEventListener('click', async () => {
      ...
      if ('Notification' in window && Notification.permission !== 'granted') {
        ...
        if (permission !== 'granted') {
          card.querySelector('#nrEnabled').checked = false;
          updateStatus('Notifications were not enabled. The reminder remains off.');
          return;
        }
      }
      ...
      if (typeof window.showToast === 'function') {
        window.showToast(`⏰ Daily reminder set for ${time}`);
      }
    });
    ...
    card.querySelector('#nrSkip').addEventListener('click', () => {
      save({
        enabled: false,
        time: card.querySelector('#nrTime').value || '21:00'
      });

      card.querySelector('#nrEnabled').checked = false;
      schedule();
      updateStatus('Reminder skipped. You can enable it anytime.');
    });

    function updateStatus(message) {
      const status = get();
      card.querySelector('#nrStatus').textContent =
        message ||
        (status?.enabled
          ? `Enabled daily at ${status.time}.`
          : 'Optional — you can leave this disabled.');
    }
  }
